...
export default function Invoices() {
    ...
    const filterInvoices = useCallback((invoices: Invoice[]): Invoice[] => {
        return invoices.filter(invoice => {
            // Filter by payment status (exclude rectified invoices and rectification invoices from payment status filtering)
            if (!showPaid && invoice.is_paid && !invoice.isRectified && invoice.total >= 0) return false;
            if (!showUnpaid && !invoice.is_paid && !invoice.isRectified && invoice.total >= 0) return false;
